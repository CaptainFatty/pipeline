pro eis_md_pipeline::perform_join, files
...
; OUT: files, count
pro eis_md_pipeline::fetch_data, files
  ...
  *self.local_logger->shell, self.sdtp + ' merge usc34 band=3 sdate=' + self.sdate + ' edate=' + self.edate + ' stime=' + self.stime + ' etime=' + self.etime
  ...
  if count eq 0 then begin
     ...
     self->log, msg
     if not self.interactive then self->update_track_file, self.sdate, self.edate
     self->exit, 99, msg
  endif

  self->perform_join, files
end
