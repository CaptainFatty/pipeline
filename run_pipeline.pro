...
; parsing orl file(s)
pro run_pipeline, start_date, end_date, start_time, end_time, flag=flag, interactive=interactive, scheduled=scheduled
...
;;;     *main_logger->log, 'EIS pipeline started interactively at ' + systime()
     ...
     *main_logger->log, 'EIS pipeline started ' + msg + systime(), /title
     ...
     if parse_date_time(sdate, edate, stime, etime) then begin
        ...
        eis_pipeline->set_date_time, sdate, edate, stime, etime
        *main_logger->log, 'run_pipeline ' + interactive_str + ' ' + flag_str + ' ' + sdate + ' ' + edate + ' ' + stime  + ' ' + etime
     endif else begin
     ...
     *main_logger->log, 'EIS pipeline started ' + msg + systime(), /title
     ...
     ; perl file will supply date/time form orl...
...
;        endif else begin
           ...
           etime = '1001'
           eis_pipeline->set_date_time, sdate, edate, stime, etime
           *main_logger->log, 'run_pipeline ' + flag_str + ' ' + sdate + ' ' + edate + ' ' + stime  + ' ' + etime
;        endelse
;     endelse
  endelse
...
;  eis_pipeline->initialise
  ...
  eis_pipeline->check_data, received_files, damaged_files
